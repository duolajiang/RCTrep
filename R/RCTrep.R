...
#'
#' @description The function \code{RCTREP} is used to replicate the estimate of treatment effect from a target randomized control trial based on non-experimental observational data. This function estimate the treatment effect of observational data to ensure the internal validity of the estimates within the study population (namely, the observational data) and weight the resulting estimates to the target population (namely, the RCT) to enable external validity. The function currently implement the following types of estimators of treatment effect: G_computation, inverse propensity score weighting (IPW), and augmented propensity score weighting. The function implement the following two types of weighting estimators to generalize the resulting estimates of treatment effect from observational data to the target randomized control trial population: exact matching weights, and selection score weights. Since the target population is the RCT, weights for each individual in observational data is \eqn{p/(1-p)}.
#'
